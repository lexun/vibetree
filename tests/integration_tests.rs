...
fn test_sync_orphaned_worktree_discovery() -> Result<()> {
    ...
    let orphaned_ports = &app.get_worktrees()["orphaned-branch"].ports;
    let normal_ports = &app.get_worktrees()["normal-branch"].ports;

    assert_eq!(orphaned_ports.len(), 2); // postgres and redis ports
    assert_ne!(
        orphaned_ports["POSTGRES_PORT"],
        normal_ports["POSTGRES_PORT"]
    );
    assert_ne!(orphaned_ports["REDIS_PORT"], normal_ports["REDIS_PORT"]);
...
fn test_sync_config_variable_changes() -> Result<()> {
    ...
    let initial_ports = app.get_worktrees()["test-branch"].ports.clone();
    assert_eq!(initial_ports.len(), 1);
    assert!(initial_ports.contains_key("POSTGRES_PORT"));
    ...
    let updated_ports = &app.get_worktrees()["test-branch"].ports;
    assert_eq!(updated_ports.len(), 2);
    assert!(updated_ports.contains_key("POSTGRES_PORT"));
    assert!(updated_ports.contains_key("REDIS_PORT"));
...
fn test_sync_main_worktree_handling() -> Result<()> {
    ...
    assert_eq!(app.get_worktrees()["main"].ports["POSTGRES_PORT"], 8000);
    ...
    let other_postgres_port = app.get_worktrees()["other-branch"].ports["POSTGRES_PORT"];
    ...
    let other_new_port = app.get_worktrees()["other-branch"].ports["POSTGRES_PORT"];
...
fn test_list_main_worktree_status() -> Result<()> {
    ...
    assert!(!main_entry.unwrap().ports.is_empty());
    assert!(!branch_entry.unwrap().ports.is_empty());
