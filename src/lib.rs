    ...
    fn setup_direnv_integration(&self, path: &std::path::Path) -> Result<()> {
        ...
        if envrc_path.exists() {
            ...
            if !existing_content.contains(&dotenv_line) {
                let new_content = format!("{}\n\n# Auto-added by vibetree\n{}\n", existing_content.trim(), dotenv_line);
                std::fs::write(&envrc_path, new_content)
                    .with_context(|| format!("Failed to update .envrc: {}", envrc_path.display()))?;
            }
        } else {
