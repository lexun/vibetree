...
impl EnvFileGenerator {
    pub fn generate_env_file(
        ...
        if let Some(parent) = env_file_path.parent() {
            fs::create_dir_all(parent).with_context(|| {
                format!("Failed to create env file directory: {}", parent.display())
            })?;
        }

        let content = Self::build_env_content(branch_name, values);
    ...
    fn build_env_content(_branch_name: &str, values: &HashMap<String, String>) -> String {
        let mut content = String::new();

        content.push_str("# Generated by vibetree - DO NOT EDIT MANUALLY\n");
        content.push_str(&format!("# Managed by: vibetree v{}\n\n", crate::VERSION));

        // Add variable values - keys are already env var names
        let mut sorted_variables: Vec<_> = values.iter().collect();
        sorted_variables.sort_by_key(|(env_var_name, _)| *env_var_name);

        for (env_var_name, value) in sorted_variables {
...
mod tests {
    ...
    fn test_generate_env_file() -> Result<()> {
        ...
        values.insert("PGPORT".to_string(), "5433".to_string());
        values.insert("REDIS_PORT".to_string(), "6380".to_string());
        values.insert("API_PORT".to_string(), "8001".to_string());

        let env_file_path = worktree_path.join(".vibetree").join("env");
        EnvFileGenerator::generate_env_file(&env_file_path, "test-branch", &values)?;
    ...
    fn test_custom_env_file_path() -> Result<()> {
        ...
        let mut values = HashMap::new();
        values.insert("API_PORT".to_string(), "8080".to_string());

        EnvFileGenerator::generate_env_file(&custom_env_path, "test-branch", &values)?;
